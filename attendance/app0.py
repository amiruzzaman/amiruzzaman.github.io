# app.py
# Attendance taker web app that writes back to the original Excel file.
...
def read_and_index_df():
    ...
    blank_ids = df['OrgDefinedId'].replace({'nan': ''}).eq('')
    if blank_ids.any():
        logging.warning("There are rows with blank OrgDefinedId.")

    dup_mask = df['OrgDefinedId'].duplicated(keep='first')
    if dup_mask.any():
        dup_vals = df.loc[dup_mask, 'OrgDefinedId'].unique().tolist()
        logging.warning("Duplicate OrgDefinedId values found: %s", dup_vals)
        df = df[~dup_mask]
...
def get_students():
    ...
    for idx, row in df.iterrows():
        ...
        if not name:
            name_parts = []
            for c in ['UserName', 'DisplayName', 'email', 'Email']:
                if c in row and pd.notna(row[c]):
                    name_parts.append(str(row[c]))
            name = " ".join(name_parts) or f"Student {idx}"
...
def update_excel_file_single(update):
    """
    Accepts a single update dict: {'student_id': '12345', 'attendance': 0/1/0.5}
    Writes attendance into a date column (YYYY-MM-DD) with background colors.
    """
    ...
    with save_lock:
        ...
        if student_id not in df.index:
            logging.warning("Student_id not found in Excel: %s", student_id)
            return {'success': False, 'message': f"student_id {student_id} not found in Excel."}

        try:
            df.at[student_id, date_col] = update.get('attendance')
        except Exception as e:
            logging.exception("Error assigning attendance")
            return {'success': False, 'message': f"Error assigning attendance: {e}"}

        try:
            if 'OrgDefinedId' not in df.columns:
                df['OrgDefinedId'] = df.index

            # Save updated values first
            df.to_excel(INPUT_EXCEL_FILE, index=False, engine='openpyxl')

            # Then reopen with openpyxl to apply formatting
            wb = load_workbook(INPUT_EXCEL_FILE)
            ws = wb.active

            # find column index of date_col
            col_idx = None
            ...
            logging.info("Saved attendance + formatting for %s -> %s", student_id, date_col)
            return {'success': True, 'message': 'Attendance saved with formatting.'}
        except Exception as e:
            logging.exception("Error saving Excel file")
...
<style>
  ...
  .card { background:white; padding:28px; border-radius:12px; box-shadow:0 8px 30px rgba(0,0,0,0.08); width:90%; max-width:720px; text-align:center; }
  ...
  @media(max-width:500px){ #name-display{font-size:1.4rem} .btn{min-width:90px} }
</style>
</head>
<body>
  <div class="card">
...
async function fetchStudents(){
  try {
    ...
    if(students.length === 0){
      nameEl.textContent = 'No students found';
...
function displayCurrent(){
  if(idx >= students.length){
    nameEl.textContent = 'Attendance complete ✅';
...
async function mark(type){
  ...
  try {
    ...
    if(!res.success){
      ...
      statusEl.textContent = 'Save failed: ' + res.message;
      setButtonsDisabled(false);
      return;
    }
    idx++;
    setTimeout(displayCurrent, 350);
  } catch(err){
    console.error('Network error', err);
    statusEl.textContent = 'Network error — retry or refresh';
    setButtonsDisabled(false);
  }
}
