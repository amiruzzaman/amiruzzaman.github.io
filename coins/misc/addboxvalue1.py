...
async function loadData() {
  ...
  allCountries = countries; // store for list view
...
function renderList() {
  ...
  let filtered = allCountries.filter(c => c.country.toLowerCase().includes(query));
  ...
  filtered.forEach(c => {
    ...
    const row = document.createElement("div");
    row.textContent = c.country + (c.box ? " (Box " + c.box + ")" : "");
