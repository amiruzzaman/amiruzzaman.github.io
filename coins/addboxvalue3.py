...
function makeCellEditable(cell, country, currentValue) {
...
async function loadData() {
  ...
  countries.forEach(c => {
    ...
    row.innerHTML = `
      ...
      <div class="cell editable-cell" data-country="${c.country}">${c.box || ''}</div>
  ...
  document.querySelectorAll('.editable-cell').forEach(cell => {
    cell.addEventListener('click', () => {
      ...
      makeCellEditable(cell, country, currentValue);
    });
  });
...
async function saveBox(countryName, boxValue) {
  if (!boxValue) {
    showMessage("Please enter a box value.", false);
    return;
  }
